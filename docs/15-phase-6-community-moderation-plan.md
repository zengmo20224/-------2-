...
- 文件上传只决定了单文件 10 MB，其余图片格式、每帖数量、目录、缩略图仍未决。

## 未决边界

`docs/08-pending-decisions.md` 中 D-007 仍有未决事项：
...
- 如果没有用户明确决定图片数量上限，生产 Controller 不应开放可滥用的多图片上传入口。
...
如果 GLM5.1 认为阶段 6 必须开放图片上传，必须先停止图片上传相关实现，列出方案并等待用户决策。
