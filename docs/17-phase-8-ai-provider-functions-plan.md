...
以下事项已于 2026-06-12 决定：

- 默认模型为 `deepseek-chat`，允许配置覆盖。
- 请求超时 30 秒。
- Provider 失败时最多重试 1 次。
- 最大输出 Token 为 2048，允许配置覆盖。
- 用户每天最多 20 次，管理员每天最多 100 次。
- V1 不启用流式输出。

这些值必须配置化并按阶段 14B 实施，不授权当前任务提前接入真实 Provider。
...
- 不允许为跑通测试绕过配置、限流、用量日志或安全规则。
- 阶段 8A 可以完成和提交，真实 Provider 保持禁用。
...
在阶段 14B 实施前，配置文件应保持真实 Provider 默认禁用；实施时使用 D-008 已决定的可覆盖默认值：
...
- 阶段 14B 实施时，配置默认值必须与 D-008 一致并允许环境覆盖。
- V1 不启用流式输出；限流配置按 D-008 已决定的用户和管理员日配额实施。
...
- 会话历史必须限制读取数量，并根据 D-008 已决定的最大输出 Token 和上下文预算配置化。
...
1. 核对 `docs/08-pending-decisions.md` 和本文件。
2. 写真实 DeepSeek Adapter 契约和错误映射测试。
3. 实现配置校验和真实 HTTP Client。
4. 实现已批准的超时、重试、Token、限流和非流式策略。
...
- 没有真实 API Key。
- 模型、超时、重试、Token 和限流均配置化，V1 不启用流式输出。
- Provider 层没有数据库依赖。
- 没有 AI 生成并执行 SQL。
- 没有把 AI 输出直接写入核心业务状态。
