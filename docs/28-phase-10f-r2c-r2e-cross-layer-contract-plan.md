...
- 禁止执行 `10F-R2C`、`10F-R2D`、`10F-R2E` 的代码修改。
- 禁止 Agent 自行采用全局 Long 序列化、BigInt 解析器或固定门店 ID。
- 禁止进入 `10F-R3` 或 `10G`。
- 只允许阅读、Review 和补充决策证据。
...
目标：向用户确认 D-011、D-012 推荐方案
允许修改：用户决定后仅更新决策和规划文档
禁止修改：所有生产代码和测试
阻塞：D-011、D-012 未决
完成条件：用户明确批准或选择替代方案，并形成独立文档提交
```

用户决定后：

- D-011 获批：下一任务只允许为 `10F-R2C1`。
- D-012 获批：下一任务只允许为 `10F-R2E1`。
- 两项均获批：仍优先执行 `10F-R2C1`，每次只领取一个任务包。
