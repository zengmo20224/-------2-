...
**状态**：首轮编码 Review 未通过；当前只能执行 11-05R。
...
当前不得与其他阶段任务并行实施；必须先关闭非 Bearer Authorization 请求头降级为匿名的问题。
...
**状态**：详细任务书已准备，当前锁定；等待 11-05R 全部门禁和 Review 通过。

**GLM5.1 详细执行任务书**：`docs/43-phase-11-06-glm5-demo-seed-data-brief.md`

**目标**：提供可重复创建和清理的演示数据方案。

**依赖**：11-01（需要测试用户身份）、11-05R（需要稳定的公开读取与认证失败语义）

**允许修改**：

- 新增 `src/main/resources/demo/` 下的种子与清理脚本
- 新增 `application-demo-seed.yml`
- 新增受 `test,demo-seed` 显式 Profile 约束的最小初始化组件
- 对应测试文件
...
1. 创建种子数据 SQL 文件，包含：
   ...
   - 基础商品分类和商品。
   - 基础话题。
2. 仅在显式组合 Profile `test,demo-seed` 下装载，默认 `test` 不自动装载。
3. 提供清理方法。
...
- 种子数据可通过脚本或命令重复创建和清理。
- 测试用户使用现有 test-login 合成手机号，不创建密码、管理员账号或真实身份信息。
- 详细固定 ID、清理边界、Profile 隔离和测试矩阵以 `docs/43-phase-11-06-glm5-demo-seed-data-brief.md` 为准。
...
- 当前只能执行 11-05R；11-06 必须等待 11-05R 门禁和 Review 通过。
